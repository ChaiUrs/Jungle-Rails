...
RSpec.describe Product, type: :model do
  ...
  describe 'Validations' do
    it 'Product can be added with name, price, quantity, and category' do
      @product = @category.products.new(name: 'Women\'s Zebra pants', price: 125, quantity: 18)
    ...
    it 'is not valid without a price' do
      @product = @category.products.new(name: 'Women\'s Zebra pants', price: nil, quantity: 18)
      expect(@product).to_not be_valid
      expect(@product.errors.full_messages.first).to eq 'Price cents is not a number'
    end

    it 'is not valid without a quantity' do
      @product = @category.products.new(name: 'Women\'s Zebra pants', price: 125, quantity: nil)
      expect(@product).to_not be_valid
